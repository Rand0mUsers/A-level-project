...
$('#newedge').on("click", function() {
    var edges = prompt("Enter comma-seperated pairs of nodes to connect e.g. A B, B C");
    ...
    // Process input...
    edges = edges.replace(/[^0-9A-Za-z ,]/g, " ") // replace anything that isn't a letter, number, comma or space with a space
        ...
        .split(/ ?, ?/); // split by commas, leaving a list of consecutive node names only

    /** Variable holding assembled error message */
    var errMessage = "";

    // For each potential edge to add:
    //   note edge given as string in format "A B", "edge1 edge2" etc
    edges.forEach(function(rawEdge) {
        // Split the string by spaces
        var edge = rawEdge.split(" ");
        if (edge.length === 2) {
            ...
            try {
                ...
                if (errMessage.length === 0) {
                    errMessage += "Invalid edge(s) given:";
                }
                errMessage += " \"" + rawEdge + "\"";
            }
        } else {
